...
export default function Home() {
  ...
  const runEnrichment = async () => {
    ...
    const initialLogs: LogEntry[] = rows.map((r, i) => ({
      index: i,
      leadId: r['Lead ID'] || `Row ${i + 1}`,
      domain: r['email domain'] || '(no domain)',
      status: 'pending',
      msg: 'Waiting…',
    }))
    setLogs(initialLogs)
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (!domain) {
        updateLog(i, { status: 'skipped', msg: 'No email domain — skipped' })
        skippedCount++
      } else if (emptyBefore === 0) {
        updateLog(i, { status: 'skipped', msg: 'All fields already filled — skipped' })
        skippedCount++
      } else {
        try {
          ...
          if (source === 'cache') {
            ...
            updateLog(i, {
              status: 'cached',
              msg: `From cache${date ? ` (${date})` : ''} · ${filled} field${filled !== 1 ? 's' : ''} filled`,
            })
          } else {
            enrichedCount++
            updateLog(i, {
              status: 'done',
              msg: `Enriched by AI · ${filled} field${filled !== 1 ? 's' : ''} filled`,
            })
          }
        } catch (err) {
          errorCount++
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F7F8FA', fontFamily: "'Segoe UI', system-ui, sans-serif" }}>
      {/* Header */}
      <header style={{
        background: '#03318C', color: 'white', height: 56,
        display: 'flex', alignItems: 'center', padding: '0 32px', gap: 16,
        boxShadow: '0 2px 8px rgba(3,49,140,0.3)',
      }}>
        <div style={{
          width: 32, height: 32, background: '#E4002B', borderRadius: 4,
        ...
        }}>Internal · Digital Marketing</span>
      </header>
          ...
          <div style={{ padding: '20px 24px' }}>
            {!csvData ? (
              <div
                ...
                style={{
                  border: `2px dashed ${dragging ? '#03318C' : '#D8DAE0'}`,
                  borderRadius: 6, padding: '40px 24px', textAlign: 'center',
                  cursor: 'pointer', position: 'relative',
                  background: dragging ? '#E8EDF8' : 'transparent',
              ...
              <>
                    ...
                    <thead>
                      <tr>
                        {csvData.headers.map((h) => (
                          <th key={h} style={{ background: '#F7F8FA', color: '#5C6070', fontWeight: 600, textTransform: 'uppercase', fontSize: 10, letterSpacing: '0.06em', padding: '9px 12px', textAlign: 'left', borderBottom: '1px solid #D8DAE0', whiteSpace: 'nowrap' }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {csvData.rows.slice(0, 5).map((row, i) => (
                        <tr key={i} style={{ borderBottom: '1px solid #EDEEF2' }}>
                          {csvData.headers.map((h) => (
                            <td key={h} style={{ padding: '8px 12px', maxWidth: 160, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', color: row[h] ? '#2A2D38' : '#D8DAE0', fontStyle: row[h] ? 'normal' : 'italic' }}>
                              {row[h] || '—'}
                            </td>
                          ))}
                        </tr>
                ...
                {csvData.rows.length > 5 && (
                  <div style={{ fontSize: 11, color: '#9EA3B0', textAlign: 'center', marginTop: 8 }}>
                ...
                <div style={{ display: 'flex', gap: 12, marginTop: 16, flexWrap: 'wrap' }}>
                  ...
                  {csvData.rows.length - leadsNeedingEnrichment > 0 && (
                    <span style={{ padding: '6px 14px', borderRadius: 20, fontSize: 12, fontWeight: 600, background: '#E6F4ED', color: '#0A7A3E' }}>✓ {csvData.rows.length - leadsNeedingEnrichment} already complete</span>
                  )}
                </div>
                ...
                <div style={{ marginTop: 20, paddingTop: 20, borderTop: '1px solid #EDEEF2', display: 'flex', alignItems: 'center', gap: 14 }}>
                  <button
                    ...
                    style={{
                      background: isRunning || leadsNeedingEnrichment === 0 ? '#9EA3B0' : '#03318C',
                      color: 'white', border: 'none', borderRadius: 6,
                      padding: '10px 22px', fontSize: 14, fontWeight: 600,
                      cursor: isRunning || leadsNeedingEnrichment === 0 ? 'not-allowed' : 'pointer',
                  ...
                  {leadsNeedingEnrichment === 0 && (
                    <span style={{ fontSize: 13, color: '#5C6070' }}>All leads already have complete data.</span>
                  )}
                </div>
              </>
            )}
              ...
              <div style={{ marginTop: 14, display: 'flex', flexDirection: 'column', gap: 4, maxHeight: 240, overflowY: 'auto' }}>
                {logs.map((l, i) => (
                  <div key={i} style={{
                    ...
                    background: l.status === 'running' ? '#E8EDF8' : l.status === 'done' ? '#E6F4ED' : l.status === 'cached' ? '#F3F0FF' : l.status === 'error' ? '#FDEAEA' : '#F7F8FA',
          ...
          <div style={{ background: 'white', border: '1px solid #D8DAE0', borderRadius: 6, boxShadow: '0 1px 4px rgba(3,49,140,0.08)' }}>
            <div style={{ padding: '20px 24px 16px', borderBottom: '1px solid #EDEEF2', display: 'flex', alignItems: 'center', gap: 10 }}>
              <div style={{ width: 32, height: 32, background: '#E6F4ED', borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <svg width="16" height="16" fill="none" stroke="#0A7A3E" viewBox="0 0 24 24">
            ...
            <div style={{ padding: '20px 24px' }}>
              ...
              <button
                onClick={downloadCSV}
                style={{
                  background: 'white', color: '#03318C', border: '1.5px solid #03318C',
