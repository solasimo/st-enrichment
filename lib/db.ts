...
export async function getDomain(domain: string): Promise<EnrichedDomain | null> {
  const rows = await sql`
    SELECT * FROM enriched_domains WHERE email_domain = ${domain} LIMIT 1
  `
...
export async function updateDomain(domain: string, fields: Record<string, string | null>): Promise<void> {
  ...
  const setClauses = entries.map(([col]) => `${col} = $${entries.indexOf([col, fields[col]]) + 2}`).join(', ')
  const values = [domain, ...entries.map(([, v]) => v)]
...
export async function getStats(): Promise<{ total: number; byYear: { year: string; count: number }[] }> {
  ...
  const byYear = rows.map((r) => ({ year: r.year as string, count: r.count as number }))
  return { total, byYear }
}
...
export async function getGoodFit(domain: string, country: string): Promise<GoodFitCache | null> {
  const rows = await sql`
    SELECT * FROM good_fit_cache
    WHERE email_domain = ${domain} AND country = ${country}
    LIMIT 1
  `
  return (rows[0] as GoodFitCache) || null
}

export async function upsertGoodFit(domain: string, country: string, goodFit: string, goodFitNotes: string): Promise<void> {
  await sql`
    INSERT INTO good_fit_cache (email_domain, country, good_fit, good_fit_notes)
    VALUES (${domain}, ${country}, ${goodFit}, ${goodFitNotes})
...
export async function getGoodFitForDomains(domains: string[]): Promise<GoodFitCache[]> {
  ...
  const rows = await sql`
    SELECT * FROM good_fit_cache WHERE email_domain = ANY(${domains})
  `
